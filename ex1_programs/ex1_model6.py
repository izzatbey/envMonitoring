...
class AnchorPositivePairs(keras.utils.Sequence):
    ...
    def __getitem__(self, _idx):
        ...
        x = np.empty((2, num_classes, IMAGE_SIZE, IMAGE_SIZE, 3), dtype=np.float32)
...
history = model2.fit(AnchorPositivePairs(num_batchs=64), epochs=1000)
model2.summary()
plt.plot(history.history["loss"])
plt.show()
model2.save("./model/ML100")
